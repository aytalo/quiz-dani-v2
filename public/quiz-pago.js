// ─── Conteúdo do quiz (versão PAGA — R$ 5) ────────────────────────────────
// Produto principal: APP DE INTELIGÊNCIA ARTIFICIAL pra roleta — você lucra
// "copiando e colando" os sinais. Fichas Douradas viram BRINDE pra começar
// a jogar sem mexer no próprio bolso.
const QUIZ = [
  {
    intro: [
      'Oi! 😊 Aqui é a Dani 🎡',
      'Antes de te liberar o *app de inteligência artificial* que eu uso pra lucrar na roleta *só copiando e colando* o sinal que aparece na tela, deixa eu te perguntar 5 coisas — em 60 segundos eu te mostro EXATAMENTE por que ele muda o seu jogo.',
      '🎁 *E pra você começar JÁ HOJE*, quem chegar até o final do quiz ganha as *FICHAS DOURADAS* de brinde pra entrar na roleta sem usar o seu próprio dinheiro. 🪙✨',
      'Bora? É rapidinho. 👇',
    ],
...
// Visit (1x) — marca essa visita como variante PAGA pra separar do quiz free
(function trackVisit() {
  const tracking = window.__TRACKING__ || JSON.parse(sessionStorage.getItem('tracking') || '{}');
  tracking._variant = 'pago-5';
  fetch('/track/visit', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  track('quiz_started', { payload: { variant: 'pago-5' } });
})();
  ...
  await new Promise(resolve => {
    renderOptions(q.options, async (idx, opt) => {
      ...
      track('question_answered', {
        ...
        payload: { option_text: opt.text, total_erros: erros, variant: 'pago-5' },
      });
      bubbleMe(opt.text);
      await sleep(420);
...
// Micro-pergunta de engajamento no meio do pitch (NÃO conta como erro)
async function microAsk(question, options, label) {
  await bubbleOther(question, 1100);
  await sleep(150);
  return new Promise(resolve => {
    renderOptions(options, async (idx, opt) => {
      track('micro_question_answered', {
        payload: { label, option_idx: idx, option_text: opt.text, variant: 'pago-5' },
      });
      bubbleMe(opt.text);
      await sleep(420);
      for (const f of (opt.reply || [])) await bubbleOther(f, 900);
      resolve({ idx, opt });
    });
  });
}
...
function buildCtaUrl() {
  const base = 'https://pay.cakto.com.br/fdxk4hz';
  const tracking = window.__TRACKING__ || JSON.parse(sessionStorage.getItem('tracking') || '{}');
  ...
  u.searchParams.set('utm_source', tracking.utm_source || 'quiz-dani-pago');
  u.searchParams.set('utm_medium', tracking.utm_medium || 'quiz');
  u.searchParams.set('utm_campaign', tracking.utm_campaign || 'roleta-app-pago-5');
  u.searchParams.set('utm_content', tracking.utm_content || 'variant-pago-5');
...
async function finalResult() {
  ...
  await sleep(300);
  await bubbleOther('Agora deixa eu te apresentar oficialmente o que vai resolver TUDO isso 👇', 1400);
  await sleep(200);
  await bubbleOther('É o *aplicativo de inteligência artificial mais completo pra roleta* que existe hoje no Brasil. 🤖🎡', 1500);
  await sleep(200);
  await bubbleOther('Ele faz a parte difícil por você. Você *só copia e cola* o sinal que aparece na tela — literalmente isso.', 1400);
  await sleep(200);
  await bubbleOther(
    'Olha o que a IA faz sozinha enquanto você só executa 👇\n' +
    '✅ Analisa 30 rodadas em segundos → diz *onde apostar*\n' +
    '✅ Sinal certo *rodada por rodada* (você só copia e cola)\n' +
    '✅ Stop-loss automático → mata o tilt sem você pensar\n' +
    '✅ Detecta mesa fria → te avisa pra *trocar ou pausar*\n' +
    '✅ Trava sozinho quando bate sua meta',
    1700
  );
  await sleep(250);
  await bubbleOther('Não é grupo de dicas. Não é sala de sinal no WhatsApp. É uma *IA* trabalhando pra você em tempo real. 📱', 1400);

  // ─── 🤝 Micro-pergunta 1: engajamento antes do preço ────────────────────
  await sleep(350);
  await microAsk(
    'Antes de te falar o valor, me responde rapidinho 👇\nFaz sentido pra você ter uma IA dando o sinal certo enquanto você só copia e cola?',
    [
      { text: 'Faz total sentido 🎯', reply: ['Era exatamente o que eu queria ouvir. 💛'] },
      { text: 'Quero ver funcionando 👀', reply: ['Boa — daqui a pouco te mostro num vídeo. Antes deixa eu te falar o valor.'] },
    ],
    'antes_preco'
  );

  // ─── 💸 Posicionamento do preço — R$ 5 ──────────────────────────────────
  await sleep(300);
  await bubbleOther('Vou ser bem direta com você. 👇', 1100);
  await sleep(200);
  await bubbleOther('Eu liberava o acesso de *graça* antes — e o problema é que a galera baixava, não dava valor, não usava direito e voltava me culpando.', 1500);
  await sleep(200);
  await bubbleOther('Então a partir de agora o acesso ao app de IA custa *R$ 5*. Cinco reais. Só isso.', 1300);
  await sleep(200);
  await bubbleOther('Por que cobrar? Pra você entrar pra valer. Quem paga 5, joga sério. Quem joga sério, copia o sinal direito e ganha. ✅', 1400);

  // ─── 🤝 Micro-pergunta 2: compromisso com o valor ───────────────────────
  await sleep(350);
  await microAsk(
    'R$ 5 pra acessar uma IA que te dá o sinal certo cabe no seu bolso hoje?',
    [
      { text: 'Cabe, é menos que um lanche 🍔', reply: ['Era o que eu pensava. 💪'] },
      { text: 'Tá tranquilo 👍', reply: ['Beleza. Então olha o BRINDE que entra junto 👇'] },
    ],
    'cabe_no_bolso'
  );

  // ─── 🎁 Brinde — fichas douradas ────────────────────────────────────────
  await sleep(300);
  await bubbleOther('E como eu quero que você COMECE a jogar hoje mesmo, eu coloquei um brinde junto 👇', 1300);
  await sleep(200);
  await bubbleOther(
    '🎁 *De brinde, junto com o acesso ao app, você recebe:*\n' +
    '🪙 *FICHAS DOURADAS* liberadas no cadastro pra você começar a jogar *sem usar o seu próprio dinheiro*.',
    1600
  );
  await sleep(200);
  await bubbleOther('*As fichas valem muito mais que R$ 5.* Ou seja: o brinde sozinho já paga o que você investiu na entrada. 💰', 1500);

  // ─── 🤝 Micro-pergunta 3: compromisso final antes do CTA ────────────────
  await sleep(350);
  await microAsk(
    'Se eu te der o app de IA + as fichas de brinde por R$ 5, você começa a jogar HOJE?',
    [
      { text: 'Tô dentro, começo hoje 🚀', reply: ['Aí sim. 🔥'] },
      { text: 'Quero ver o passo a passo 🎬', reply: ['Beleza — daqui a pouco te mostro o vídeo com o passo a passo.'] },
    ],
    'comeca_hoje'
  );

  // ─── 📦 Resumo + escassez + vídeo + CTA ─────────────────────────────────
  await sleep(300);
  await bubbleOther('Resumindo o que entra na sua mão por R$ 5 👇', 1200);
  await sleep(150);
  await bubbleOther(
    '✅ Acesso completo ao *APP de IA da Dani* — você só copia e cola o sinal e lucra\n' +
    '✅ Sinais por rodada, stop-loss automático, leitura de mesa em tempo real\n' +
    '🎁 *Brinde:* *Fichas Douradas* pra começar a jogar sem mexer no seu bolso\n' +
    '📲 Suporte direto comigo caso trave em qualquer parte',
    1800
  );
  await sleep(250);
  await bubbleOther('⏰ *Só vale hoje.* Amanhã o brinde das fichas sai do ar e o acesso volta a ser só o app, sem ficha de presente.', 1500);
  await sleep(200);
  await bubbleOther('Gravei esse vídeo rapidinho te mostrando *exatamente* como pagar os R$ 5, ativar o app e resgatar as fichas — assiste agora 👇', 1300);
  await sleep(150);
  await bubbleVideo('/fichas-douradas.mp4', '🎥 Como liberar o app de IA + resgatar as fichas douradas', 900);
  await sleep(300);
  await bubbleOther('Toca no botão aqui embaixo, paga os R$ 5, o app é liberado na hora e as fichas caem junto. 👇', 1400);

  await sleep(300);
  await showTyping(500);
  track('quiz_finished', { payload: { score: erros, variant: 'pago-5' } });
  ...
  cta.target = '_blank';
  cta.rel = 'noopener';
  cta.innerHTML = '🚀 Quero o app por R$ 5 + fichas de brinde →';
  cta.addEventListener('click', () => {
    cta.href = buildCtaUrl();
    track('cta_clicked', { payload: { score: erros, href: cta.href, variant: 'pago-5' } });
    if (typeof window.fbq === 'function') window.fbq('track', 'Lead', { content_name: 'quiz-dani-cta-pago', value: 5, currency: 'BRL' });
  });
